...
class ShellInstance {
    ...
    void installRequires(List<String> pythonRequires, { bool echo = true }) {
        ...
        Process.runSync(pythonPath, [ "-m", "pip", "install", "-r", tempPythonRequireFile ]);
        File(tempPythonRequireFile).deleteSync();
        if (echo) print("Requirements installed.");
    }

    Future<void> runFile(String pythonFile, { String? workingDirectory, ShellListener? listener, bool echo = true }) async {
        ...
        Process process = await Process.start(pythonPath, [ "-u", pythonFile ], mode: ProcessStartMode.normal, workingDirectory: defaultWorkingDirectory ?? workingDirectory);
    ...
    Future<void> runString(String pythonCode, { String? workingDirectory, ShellListener? listener, bool echo = true }) async {
        ShellListener newListener = listener ?? ShellListener();

        String tempPythonFile = path.join(tempDir, "${DateFormat("yyyy.MM.dd.HH.mm.ss").format(DateTime.now())}.py");
        File(tempPythonFile).writeAsStringSync(pythonCode);
        ShellListener listenerToSend = ShellListener(
            onMessage: (message) {
                newListener.onMessage(message);
            },
            onError: (e, s) {
        ...
        await runFile(tempPythonFile, workingDirectory: workingDirectory, listener: listenerToSend, echo: echo);
    }
