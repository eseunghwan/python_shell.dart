...
class ShellManager {
    static ShellInstance createInstance({ String? instanceName, List<String>? pythonRequires, bool echo = true }) {
        ...
        if (Directory(instanceDir).existsSync()) {
            ...
            var instance = ShellInstance(name: instanceName, pythonPath: envPython, tempDir: tempDir);
    ...
    static ShellInstance getInstance(String instanceName) {
        ...
        if (Directory(instanceDir).existsSync()) {
            return ShellInstance(
                name: instanceName,
                pythonPath: Platform.isWindows ? path.join(instanceDir, "env", "Scripts", "python.exe") : path.join(instanceDir, "env", "bin", "python"),
                tempDir: path.join(instanceDir, "temp")
            );
        }
        else {
            return ShellManager.createInstance(instanceName: instanceName, echo: false);
        }
    }
